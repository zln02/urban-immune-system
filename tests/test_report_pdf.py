...
def test_delta_color_up():
    from backend.app.services.report_pdf import C_RED, _delta_color

    assert _delta_color(60.0, 50.0) == C_RED


def test_delta_color_down():
    from backend.app.services.report_pdf import C_GREEN, _delta_color
...
def test_md_to_paragraphs_headings():
    ...
    from backend.app.services.report_pdf import _FONT, _FONT_B, _md_to_paragraphs
...
def test_build_alert_pdf_returns_bytes():
    ...
    async def run():
        with (
            patch.object(report_pdf, "_build_pdf_story", side_effect=fake_build_pdf_story),
