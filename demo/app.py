...
def main() -> None:
    ...
    with tab2:
        ...
        st.info(
            "🧪 **합성 시나리오 (게이트 동작 시연용)** — 이 화면에는 단일 계층만 "
            "급등하는 상황을 **의도적으로 심어 두었다.** 실제 데이터에서 관측된 "
            "사건이 아니라, 게이트가 무엇을 걸러내는지 보여주기 위해 만든 "
            "시나리오다.",
            icon="🧪",
        )
        st.markdown(
        ...
        st.caption(
            "위 그래프는 **합성 시나리오(게이트 동작 시연용)** 다. "
            "X 표시가 찍힌 시점은 언론 보도로 검색만 폭증하거나 할인 행사로 "
            "약국 판매만 뛰는 상황을 모사해 넣은 지점이다."
        )
        ...
        st.caption(
            "위 3개 수치는 **합성 시나리오(게이트 동작 시연용)** 를 17개 지역에 "
            "걸쳐 합산한 것이다. 급등 사건을 인위적으로 심은 데이터이므로 "
            "**실제 성능 지표가 아니고, 실제 오경보율로 읽어서도 안 된다.** "
            "게이트가 무엇을 하는지 보여주는 용도다."
        )
