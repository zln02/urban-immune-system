...
export const GLOSSARY: Record<GlossaryKey, { title: string; body: string }> = {
  ...
  leadtime: {
    ...
    body: "임상 신고(KCDC)가 늘어나기 며칠/몇 주 전에 우리 신호가 먼저 올랐는지. 양수일수록 좋음. 17지역 평균 6.76주(약 47일) 선행.",
  },
  f1: {
    title: "F1 점수",
    body: "정밀도와 재현율을 합친 종합 정확도 (0~1). 1에 가까울수록 좋음. 17지역 walk-forward 평균 0.907 — 단 OTC 기반 self-proxy 라벨 기준이라 KDCA 확진 라벨과는 별도 검증 중.",
  },
  recall: {
    title: "재현율 (Recall)",
    body: "진짜로 발생한 사건 중 시스템이 잡아낸 비율. 17지역 평균 0.882 = 진짜 중 약 88% 포착.",
  },
  precision: {
    title: "정밀도 (Precision)",
