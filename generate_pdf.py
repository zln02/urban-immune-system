...
def build_pdf():
    ...
    # ──────────────────────────────────────────────────────────────
    # PAGE 14: 상용화 가치 및 수익성 추정
    # ──────────────────────────────────────────────────────────────
    pdf.add_page()
    pdf.chapter_title("10. 상용화 가치 및 수익성 추정", 1)
    pdf.body_text(
        "아래 수치는 확정 계약가가 아니라, 현재 저장소 구조를 실제 서비스로 운영한다고 가정한 "
        "보수적 사업성 시나리오다. 서울 25개 보건소를 초기 시장으로, 전국 252개 보건소를 "
        "확장 하한선으로 두고 계산했다."
    )
    ...
    pdf.info_box(
        "해석",
        [
            "기관당 월 300만원만 확보해도 서울 25개 보건소 기준 흑자 구조가 가능",
            "기준 시나리오(전국 252개, 월 300만원) 연 매출은 약 90억 7,200만원",
            "즉, 서울 시범사업이 성공하면 전국 확장 시 수십억 원대 ARR 사업으로 커질 수 있음",
        ],
        (235, 250, 235)
    )
    ...
    pdf.body_text(
        ...
        "OpenAI GPT-4.1 공식 가격표. 인건비와 공통 고정비는 내부 보수 추정치다."
    )
