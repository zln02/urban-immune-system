...
def train(
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(tscv.split(X)):
        ...
        if len(np.unique(y_label_val)) < 2 or len(np.unique(y_pred_label)) < 2:
            ...
            precision = float(precision_score(y_label_val, y_pred_label, zero_division=0))
    ...
    if valid_scores:
        ...
        summary["cv_mean_auc"] = float(np.mean([s["auc_roc"] for s in valid_scores if not np.isnan(s["auc_roc"])]))
