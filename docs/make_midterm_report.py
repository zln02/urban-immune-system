...
OUTPUT = os.path.join(os.path.dirname(__file__), "UIS_중간보고서_2026_v2.docx")
...
r = title_p.add_run("설계(프로젝트) 중간보고서")
...
for b in [
    ...
    "17개 시·도 26주 walk-forward 백테스트: F1=0.907, Recall=0.882, FAR=0.250, Lead Time=6.76주",
]:
    add_bullet(doc, b)

add_p(doc, "다. 선행연구 및 제품 관련 자료 조사", bold=True, size=12, space_before=6, space_after=4)
for b in [
    "Lee et al. (2023, Nature): OTC 구매×하수 2계층 융합, 단일 지역 Lead 1~2주 → UIS는 3계층으로 확장, 17개 지역 커버",
    "KOWAS (2023, 질병관리청): 하수 단독 감시, Lead 2~3주 → UIS는 L1·L3 추가 교차검증으로 FAR 개선",
    "Macao XGBoost (2024): 다중 신호 XGBoost 앙상블, F1=0.85 → UIS는 Gate B 규칙 추가로 F1=0.907 달성",
...
for b in [
    ...
    "RAG(검색 증강 생성) 기반 Claude/GPT-4o 자연어 경보 리포트 자동 생성으로 비전문가 보건당국 활용 지원",
]:
    add_bullet(doc, b)
...
for b in [
    ...
    "Gate B 교차검증 규칙: 2-of-3 신호 동시 초과 시에만 경보 발령 → FAR 58.5% 감소, Recall −2.4%의 최적 trade-off",
    "지역별 임계값 차등 보정: 약신호 지역(충북·대구·경북) 별도 임계값 설정으로 알고리즘 수정 없이 recall 향상",
    "RAG 자연어 리포트: Qdrant 벡터 검색 + LLM으로 경보 발령 근거를 비전문가도 이해 가능한 보고서로 자동 생성",
    "B2G 지향 설계: 질병관리청·지자체 납품 목표, OpenAPI 문서화 및 K8s 배포 매니페스트 완비",
]:
    add_bullet(doc, b, indent=1.0)

add_p(doc, "2) 기대효과", bold=True, size=11, indent=0.5, space_before=4, space_after=3)
for b in [
    "보건당국의 감염병 유행 인지 시점을 기존 임상 확진 대비 평균 6.76주 앞당겨 선제적 격리·방역 대응 가능",
...
add_p(doc, "1) 전체 추진일정: 2026년 3월 2일 ~ 2026년 6월 13일", size=11, indent=0.5, space_after=3)
add_p(doc, "2) 추진항목별 일정", size=11, indent=0.5, space_before=2, space_after=4)

sched_t = doc.add_table(rows=9, cols=7)
sched_t.style = "Table Grid"
sched_headers = ["추진항목", "3~4주", "5~6주", "7~8주", "9~10주", "11주", "진척도"]
...
for b in [
    ...
    "L2(하수): KOWAS 주간 PDF 파싱, 주 1회(화 10:00), 인플루엔자 RT-PCR copies/mL, 17개 시·도",
    "L3(검색): 네이버 DataLab API, 주 1회(월 09:05), 키워드 5종(독감증상·인플루엔자·타미플루 등)",
    "모든 신호 Min-Max 정규화(0-100) 후 TimescaleDB 하이퍼테이블(파티션: weekly) 적재",
]:
    add_bullet(doc, b)
...
for b in [
    ...
    "ML 모델 3종: XGBoost 앙상블(주모델, F1=0.907), TFT 7/14/21일 예측(PoC, val_loss=5.48 Phase3 재학습 예정), Autoencoder 이상탐지(99p 임계값)",
    "지역별 임계값 보정: 충북·대구·경북 composite-level 임계값 하향 조정 → 충북 recall 0.529→0.941 향상",
    "API 설계: FastAPI :8001, 라우터 17개 (signals·predictions·alerts·chat·health), SSE 실시간 경보 스트림",
]:
    add_bullet(doc, b)
...
r = arch.add_run(
    ...
    "L2 KOWAS PDF  ─┼─→  TimescaleDB  ─→  Gate B 교차검증  ─→  FastAPI :8001\n"
    "L3 Naver Data ─┘    (하이퍼테이블)        Region-tiered 보정      SSE Alerts\n"
    "                    Kafka KRaft            Anomaly Detector       Next.js Dashboard\n"
    "                    Qdrant (RAG)           RAG (Claude/GPT-4o)    PDF Report"
)
r.font.name = "Courier New"
r.font.size = Pt(9)

add_p(doc, "라. 재료비 산출 및 부품리스트", bold=True, size=12, space_before=6, space_after=4)
cost_t = doc.add_table(rows=6, cols=5)
cost_t.style = "Table Grid"
for i, h in enumerate(["NO", "품명·서비스", "규격", "수량", "금액(원/월)"]):
    set_cell(cost_t.rows[0].cells[i], h, bold=True, bg="BDD7EE", align=WD_ALIGN_PARAGRAPH.CENTER, size=9)
cost_data = [
    ("1", "GCP e2-standard-2 (VM)", "2vCPU·8GB·50GB SSD", "1", "~50,000"),
    ("2", "네이버 API (쇼핑인사이트·DataLab)", "공개 API, 월 쿼터 이내", "2종", "0"),
    ("3", "OpenAI API (GPT-4o, RAG 폴백)", "종량제", "데모용", "~5,000"),
    ("4", "Claude API (Haiku, RAG 기본)", "종량제", "데모용", "~3,000"),
    ("합계", "", "", "", "~58,000"),
]
for i, row_d in enumerate(cost_data):
    for j, v in enumerate(row_d):
        bold = (i == 4)
        bg = "BDD7EE" if i == 4 else None
...
tool_t = doc.add_table(rows=9, cols=2)
tool_t.style = "Table Grid"
set_cell(tool_t.rows[0].cells[0], "NO  사용도구", bold=True, bg="BDD7EE", align=WD_ALIGN_PARAGRAPH.CENTER, size=9)
set_cell(tool_t.rows[0].cells[1], "활용 내역", bold=True, bg="BDD7EE", align=WD_ALIGN_PARAGRAPH.CENTER, size=9)
tools = [
    ("Python 3.11 / FastAPI / Pydantic / SQLAlchemy", "백엔드 API 서버, DB ORM, 환경변수 검증"),
    ("XGBoost / PyTorch Lightning / TFT / Autoencoder", "앙상블 경보 모델, 시계열 예측, 이상탐지"),
    ("Apache Kafka KRaft / TimescaleDB / Qdrant", "실시간 스트리밍, 시계열 DB 하이퍼테이블, 벡터 검색"),
    ("Next.js 14 / Deck.gl / Naver Maps API", "대시보드, 17개 시·도 지도 시각화"),
    ("Docker Compose / GitHub Actions CI (6잡)", "로컬 인프라, lint·test·coverage 자동화"),
    ("ruff / mypy / pytest (113단위+19통합)", "코드 품질, 타입 검사, 커버리지 ≥35%"),
    ("Claude Code / GPT-4o", "코드 리뷰 보조, RAG 보고서 생성"),
    ("python-docx / pdfplumber / Selenium", "문서 자동화, KOWAS PDF 파싱, 크롤링"),
]
...
for b in [
    "3계층 실데이터 수집 파이프라인 구현 완료 (L1·L3 자동 수집, L2 수동 PDF → Phase 3 자동화 예정)",
    "XGBoost 17지역 walk-forward 백테스트: F1=0.907 달성 (목표 0.80 초과 달성)",
    "FAR 0.250 달성 (목표 0.30 미만 충족), Gate B 효과 58.5% FAR 개선·Recall −2.4% 최적 trade-off",
    "Next.js 대시보드 + SSE 실시간 경보 + RAG 자연어 리포트 + PDF 다운로드 기능 구현 완료",
    "pytest 단위 113건·통합 19건, CI 6잡(lint·test·coverage≥35%) 모두 통과, K8s 보안 컨텍스트 완비",
]:
    add_bullet(doc, b)
...
for b in [
    ...
    "L2 KOWAS PDF 수동 추출 → Selenium 자동 크롤링 파이프라인으로 전환 필요 (Phase 3 최우선 과제)",
    "TFT 실데이터 모델이 26주 데이터 부족으로 val_loss=5.48 발산 → 12주 추가 누적 후 재학습 예정",
    "단일 시즌(2025~2026 인플루엔자) 데이터로 일반화 한계 → 2시즌 이상 누적 후 외부 검증 필요",
    "RAG 리포트 생성 품질에 대한 BLEU/ROUGE 정량 평가 미완 → 향후 자동 평가 파이프라인 구축 필요",
]:
    add_bullet(doc, b)

add_p(doc, "다. 향후 과제", bold=True, size=12, space_before=6, space_after=4)
for b in [
    "Phase 3: KOWAS 자동 크롤링 구현, HIRA OpenAPI 연동(L1·L3 지역 분리), TFT 데이터 누적 재학습",
...
for s in [
    ...
    "프로젝트 GitHub 저장소 (비공개, 최종발표 후 공개 예정) — https://github.com/zln02/urban-immune-system",
]:
