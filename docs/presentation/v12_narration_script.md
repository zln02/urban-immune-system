...
> "약국 OTC 1~2주 선행. 하수 바이오마커 **2~3주 선행** (가장 빠름). 검색 트렌드 1~2주 선행.
...
> "숫자를 있는 그대로. **6.76주 선행 평균**, 17지역 walk-forward.
> F1 0.907 · Precision 0.940 · Recall 0.882 · AUC-ROC 0.931 · **오경보율 0.250** — 모두 캡스톤 기준 F1≥0.80 / FAR<0.30 충족.
...
> "**한 시스템, 세 질병 검증.** 캡스톤 평가 4번째 항목 '확장성' 입증.
> 인플루엔자 0.907 (V11.6 KDCA 0.96).
> [ COVID 클릭 ] COVID-19 F1=0.68. OTC 신호 약하고 변이 분산. transition target 으로 0.55→0.68.
> [ 노로 클릭 ] 노로 F1=0.70. 단기 폭발 패턴. transition target 이 level 대비 ML 우위 **+13%p**, FAR=0.107.
...
> 팀 5인 — PM/AI 박진영, Backend 이경준, Data 이우형, Frontend 김나영, DevOps 박정빈. Claude Code Opus·Sonnet 정직 사용했고 핵심 설계는 모두 사람."
...
# 3. 슬라이드 deck 사전 로딩
http://34.47.113.176/slides → 첫 화면 (S01) 로딩 확인 — babel transform 1~2초
